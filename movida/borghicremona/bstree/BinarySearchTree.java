		...
		public KeyValueElement getEntry() {
			return this.entry;
		}
	...
	private static Object[] recursiveToArray(List list, Node node) {
		if (null != node) {
			list.add(node.getEntry().getValue());	

			BinarySearchTree.recursiveToArray(list, node.getLeftChild());
			BinarySearchTree.recursiveToArray(list, node.getRightChild());
		}

		return list.toArray();
	}
	public Object[] toArray() {
		if (null == this.root) return new Object[]{};
		List<Object> l = new ArrayList<>();
		
		return BinarySearchTree.recursiveToArray(l, this.root);
	}	
